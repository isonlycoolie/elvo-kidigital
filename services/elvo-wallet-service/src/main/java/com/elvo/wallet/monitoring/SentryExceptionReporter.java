...
public class SentryExceptionReporter {
    ...
    public SentryExceptionReporter(@Value("${elvo.monitoring.sentry.service-tag:wallet-service}") String serviceTag) {
        this.serviceTag = serviceTag;
    }

    public void captureCriticalException(Throwable throwable,
                                         @Nullable HttpServletRequest request,
                                         @Nullable Map<String, Object> extraContext) {
    ...
    private void enrichFromRequest(IScope scope, @Nullable HttpServletRequest request) {
        String requestId = MDC.get(CorrelationIdFilter.REQUEST_ID_KEY);
        String correlationId = MDC.get(CorrelationIdFilter.CORRELATION_ID_KEY);
        if (request != null) {
            if (requestId == null || requestId.isBlank()) {
